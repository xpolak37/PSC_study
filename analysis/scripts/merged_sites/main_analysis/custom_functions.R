...
merging_data <- function(asv_tab_1, asv_tab_2,
  ...
  if (!is.null(asv_tab_2)){
    ...
    else if (segment=="colon")  metadata_2 <- metadata_2[metadata_2$Matrix %in% c("Cecum","Rectum","CD","CA","CD","SI"),]
...
pairwise.lm <- function(formula,factors,data, p.adjust.m ='BH')
{
  ...
  if ("ALD post_ltx" %in% factors) {
    co <- data.frame("1"=c("HC","ALD post_ltx","HC"),
                     "2"=c("ALD post_ltx","PSC post_ltx","PSC post_ltx")) %>% t()
  }
  models <- c()
  names <- c()
  emeans_models <- c()
...
horizontal_barplot <- function(wb,taxa){
  ...
  for (name in sheets_names){
    ...
    group1_name <- gsub(" ",".",group1_name)
    group2_name <- gsub(" ",".",group2_name)
  ...
  if (any(grepl("ALD",colnames(prevalences_df)))){
    prevalences_df <- prevalences_df %>%
      group_by(SeqID) %>%
      summarise(
        PREVALENCE_.HC = coalesce(PREVALENCE_.HC[1], PREVALENCE_.HC[2]),
        PREVALENCE_.ALD.post_ltx = coalesce(PREVALENCE_.ALD.post_ltx[1], PREVALENCE_.ALD.post_ltx[2]),
        PREVALENCE_.PSC.post_ltx = coalesce(PREVALENCE_.PSC.post_ltx[1], PREVALENCE_.PSC.post_ltx[2])
      )
  } else if (any(grepl("post_ltx",colnames(prevalences_df)))){
    prevalences_df <- prevalences_df %>%
      group_by(SeqID) %>%
      summarise(
  ...
  prevalences_df_melt <- prevalences_df_melt %>%
    mutate(variable = case_when(
      ...
      variable == "PREVALENCE_.PSC.post_ltx" ~ "PSC post_LTx",
      variable == "PREVALENCE_.ALD.post_ltx" ~ "ALD post_LTx",
      variable == "PREVALENCE_.HC" ~ "HC",
  ...
  } else if ("ibd" %in% prevalences_df_melt$variable){
    colors <- c("#A06A2C", "#B2182B")  
    
    prevalences_df_melt$variable <- factor(prevalences_df_melt$variable,levels = c("no_ibd","ibd"))
  } else if ("ALD post_LTx" %in% prevalences_df_melt$variable) {
    colors <- c("#309f87", "#DDCC77","#425387")  
    prevalences_df_melt$variable <- factor(prevalences_df_melt$variable,levels = c("HC","ALD post_LTx","PSC post_LTx"))
  } else (cat("chyba","\n"))
...
alpha_diversity_countries <-function(alpha_data,show_legend=FALSE, 
  ...
  if ("post_LTx" %in% alpha_data$Group) {
    ...
    #colors <- c("#1B7837","#B2182B")  
    #colors <- c("#D04E36", "#3F7D3C")  
    colors <- c("#5AAEAC", "#7B3294")  
    
    alpha_data$Group <- factor(alpha_data$Group,levels = c("low","high"))
  } else if ("ALD post_ltx" %in% alpha_data$Group){
    #colors <- c("#1B7837","#B2182B")  
    #colors <- c("#D04E36", "#3F7D3C")  
    colors <- c("#ae8b0c", "#425387","#309f87")  
    
    alpha_data$Group <- factor(alpha_data$Group,levels = c("ALD post_ltx",
                                                           "PSC post_ltx",
                                                           "HC"  ))
  }  
  else (print("chyba","\n"))
  ...
  p_richness <- ggplot(alpha_richness_data, aes(x=Group, y=value, fill=Country)) + 
    ...
    geom_jitter(aes(x=Group, y=value, color=Group, shape=Country), 
                position=position_jitterdodge(jitter.width=jitter, dodge.width=0.8), 
                size=size) + 
    scale_color_manual(values=colors) +
    scale_fill_manual(values=c("#ffffff","#ffffff")) +
    theme_minimal() + 
...
pca_plot_custom <- function(asv_table,taxa_table,metadata, 
  ...
  if (variable=="Group"){
    if ("post_LTx" %in% metadata[,variable]) {
      ...
      #colors <- c("#D04E36", "#3F7D3C")  
      colors <- c("#ae8b0c", "#425387","#309f87")   
      metadata[,variable] <- factor(metadata[,variable],levels = c("ALD post_ltx",
                                                                   "PSC post_ltx",
                                                                   "HC"  ))
    }
...
pca_plot_corr <- function(asv_table,taxa_table,metadata, 
 ...
 if (clinical){
  ...
  if (segment=="terminal_ileum"){
    ...
    mdi_bact <- read.xlsx("../../../results/merged_sites/main_analysis/Q1/univariate_analysis/psc_effect_terminal_ileum.xlsx")$SeqID
    
    
  } else if (segment=="colon"){
    samples_to_test <- rownames(my_df)
    mdi_bact <- read.xlsx("../../../results/merged_sites/main_analysis/Q1/univariate_analysis/psc_effect_colon.xlsx")$SeqID
...
dot_heatmap_linda <- function(raw_linda, uni_list,
  ...
  if (class(raw_linda)=="data.frame") {
    ...
    names(raw_linda) <- gsub("(terminal_ileum)|(colon)|(ASV)|(genus)|(phylum)","",names(raw_linda))
...
clinical_correlation <- function(metadata,variable,level,
  ...
  if (segment=="colon"){
    ...
    r_values_list <- unlist(purrr::map(corrs, ~ .x$r))  # List of r matrices
    p_values_list <- unlist(purrr::map(corrs, ~ .x$P))  # List of P matrices
