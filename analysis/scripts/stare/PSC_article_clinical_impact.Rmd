---
title: "PSC study: clinical analysis"
output:
  html_document:
    df_print: paged
    toc: true
...
metadata_ileum$Country <- factor(metadata_ileum$Country,levels = c('CZ','NO'))
ileum_mdi_boxplot <- melt(metadata_ileum %>%
                           mutate(`MDI`=dys_filtered_genus)) %>% dplyr::filter(variable=="MDI") %>% dplyr::filter(!is.na(value))
  
p_il_dys_genus <- ggplot(ileum_mdi_boxplot) + 
...
p_il_dys_genus <- ggplot(ileum_mdi_boxplot, aes(x=Group, y=value, fill=Country)) + 
...
```{r, warning=FALSE}
metadata_for_boxplots %<>% dplyr::mutate(Group = case_when(
      Group == "rPSC" ~ "post_LTx",
...
for (clinical_variable in variables){
  ...
  df$Group <- factor(df$Group,levels=c("pre_ltx","post_LTx"))
  df <- df %>% drop_na()
  p_test <- pairwise.wilcox.test(df[,clinical_variable] %>% filter(Group == "pre_LTx"), df$Group,
                 p.adjust.method = "BH")$p.value
  
  if (all(colnames(p_test) == c("healthy","non-rPSC","rPSC")) &
      all(rownames(p_test) == c("non-rPSC","rPSC","pre_ltx"))){
    test_df[i,] <- c(clinical_variable,
                     ...
                     round(p_test[3,3],3),
                     round(p_test[2,2],3))
  } else if (all(colnames(p_test) == c("non-rPSC","rPSC")) &
      all(rownames(p_test) == c("rPSC","pre_ltx"))) {
    test_df[i,] <- c(clinical_variable,NA,NA,NA,
                   round(p_test[2,1],3),round(p_test[2,2],3),round(p_test[1,],3))
  } else{
...
for (clinical_variable in variables){
  ...
  df <- df %>% drop_na()
  p_test <- pairwise.wilcox.test(df[,clinical_variable], df$PSC_IBD,
                 p.adjust.method = "BH")$p.value
...
# PCA and clinical variables

```{r}
variables <- c("Bilirubin","Albumin","ALP","Platelets","Calprotectin","AOM","APRI","INR","FIB4","Creatinine","ALT","AST","GGT","Nancy_max","eMAYO","MAYO_dai")
```
...
```{r}
metadata_ileum_pca <- metadata_ileum %>% 
  dplyr::select(SampleID, Group, Country,variables) 

p_1 <- pca_plot_custom(filt_ileum_genus_tab,
...
```{r}
metadata_ileum_pca <- merge(filt_ileum_metadata, 
                            metadata_ileum[,c("SampleID", "Calprotectin", "Nancy_max", "MAYO_dai")],by="SampleID",
                            all.x = TRUE) %>% 
  dplyr::mutate(log_Calprotectin=log(Calprotectin)) %>% 
  `rownames<-`(NULL)

metadata_ileum_pca <- metadata_ileum_pca[match(filt_ileum_metadata$SampleID, metadata_ileum_pca$SampleID), ] %>% `rownames<-`(NULL)

p_2 <- pca_plot_custom(filt_ileum_genus_tab,
                     filt_ileum_genus_taxa,
                     metadata_ileum_pca,
                     show_boxplots = FALSE,
...
p_3 <- pca_plot_custom(filt_ileum_genus_tab,
                     filt_ileum_genus_taxa,
                     metadata_ileum_pca,
                     show_boxplots = FALSE,
                     variable = "Nancy_max", size=3, 
                     show_legend= TRUE)
p_3
```

```{r}
p_4 <- pca_plot_custom(filt_ileum_genus_tab,
                     filt_ileum_genus_taxa,
                     metadata_ileum_pca,
                     show_boxplots = FALSE,
                     variable = "MAYO_dai", size=3, 
                     show_legend= TRUE)
...
```{r}
p_ileum <- ggarrange(p_1,p_2,p_3,p_4,ncol = 1)
```
...
```{r,warning=FALSE}
set.seed(123)
colon_metadata_pca <- colon_metadata %>%
  group_by(Patient) %>%
  distinct(Patient, .keep_all = TRUE) %>%
  as.data.frame() %>% `rownames<-`(NULL)

colon_asv_pca <- colon_asv_tab %>% dplyr::select("SeqID",colon_metadata_pca$SampleID)

colon_taxa_pca <- colon_taxa_tab %>% 
  dplyr::filter(SeqID %in% colon_asv_pca$SeqID)

data_checked <- data_check(colon_asv_pca,colon_taxa_pca)
colon_asv_pca <- data_checked[[1]]
colon_taxa_pca <- data_checked[[2]]

# Aggregation
genus_data <- aggregate_taxa(colon_asv_pca,
                             colon_taxa_pca,
                             taxonomic_level="Genus",
                             names=TRUE)
# Filtration
...
```{r}
metadata_colon_pca <- metadata_colon %>% 
  dplyr::select(SampleID, Group, Country, PatientID,variables) %>% 
  dplyr::filter(SampleID %in% filt_colon_metadata$SampleID) %>% `rownames<-`(NULL)

p_1 <- pca_plot_custom(filt_colon_genus_tab,
...
```{r}
metadata_colon_pca <- merge(filt_colon_metadata, metadata_colon[,c("SampleID","Calprotectin","Nancy_max","MAYO_dai")],
                            by="SampleID",all.x = TRUE) %>% 
  dplyr::mutate(log_Calprotectin=log(Calprotectin)) %>% `rownames<-`(NULL)
...
p_3 <- pca_plot_custom(filt_colon_genus_tab,
                     filt_colon_genus_taxa,
                     metadata_colon_pca,
                     show_boxplots = FALSE,
                     variable = "Nancy_max", size=3, 
...
p_4 <- pca_plot_custom(filt_colon_genus_tab,
                     filt_colon_genus_taxa,
                     metadata_colon_pca,
                     show_boxplots = FALSE,
                     variable = "MAYO_dai", size=3, 
...
```{r,warning=FALSE}
p_colon <- ggarrange(p_1,p_2,p_3,p_4,ncol = 1)
```

```{r}
p <- ggarrange(p_ileum + ggtitle("Terminal ileum"),
               p_colon + ggtitle("Colon"),ncol = 2)
```

```{r, warning=FALSE, fig.width=10,fig.height=15, warning=FALSE}
p
```

```{r, warning=FALSE, results='hide', echo=FALSE}
pdf("../figures/clinical/pca_envfit.pdf",
    height =15,width = 10)
p
dev.off()
```
...
```{r}
cor_res <- cor.test(metadata_for_boxplots$Calprotectin,metadata_for_boxplots$Nancy_max)
cor_res
```
...
p_1 <- ggplot(metadata_for_boxplots) + 
    geom_jitter(aes(x=Nancy_max, y=Calprotectin, color=Group),height = 0,width = 0.2) + 
  geom_text(aes(x=3.2, y=5700),
              label=gsub("\\\\n", "\n", labels),hjust = 0) + 
    geom_smooth(aes(x=Nancy_max, y=Calprotectin),method=lm, se=TRUE) +
    theme_minimal() + 
    theme(panel.border = element_rect(color = "black", fill = NA, size = 0),
          axis.ticks.x = element_line(size=0.3,color = "black"),
          axis.ticks.y = element_line(size=0.3,color="black"),
...
```{r}
cor_res <- cor.test(metadata_for_boxplots$Calprotectin,metadata_for_boxplots$ALP)
cor_res
```
...
```{r}
cor_res <- cor.test(metadata_for_boxplots$Calprotectin,metadata_for_boxplots$GGT)
cor_res
```
...
p_3 <- ggplot(metadata_for_boxplots) + 
    geom_jitter(aes(x=GGT, y=Calprotectin, color=Group),height = 0,width = 0) + 
...
```{r}
cor_res <- cor.test(metadata_for_boxplots$ALP,metadata_for_boxplots$Nancy_max)
cor_res
```
...
p_4 <- ggplot(metadata_for_boxplots) + 
    geom_jitter(aes(x=Nancy_max, y=ALP, color=Group),height = 0,width = 0.2) + 
...
```{r}
cor_res <- cor.test(metadata_for_boxplots$GGT,metadata_for_boxplots$Nancy_max)
cor_res
```
