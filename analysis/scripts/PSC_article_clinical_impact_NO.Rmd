---
title: "PSC, clinical impact - NORWAY"
output:
  html_document:
    df_print: paged
    toc: true
...
```{r}
metadata_no %<>% dplyr::mutate(ALT=ALAT,
                               AST=ASAT) %>%
  dplyr::select(-c(ALAT,ASAT))
```
...
```{r}
metadata_no$Calprotectin[metadata_no$Group=="healthy"] <- NA
metadata_ileum <- metadata_no %>% subset(Matrix=="TI") %>% as.data.frame()
metadata_colon <- metadata_no %>% subset(Matrix!="TI") %>% as.data.frame()

metadata_for_boxplots <- metadata_no[,variables] %>%
  group_by(PatientID) %>%
...
```{r,fig.width=20, fig.height=8,warning=FALSE}
...
```{r}
results_model <- pairwise.lmer(
  formula = "Shannon ~ Group + (1|PatientID)",
  factors=metadata_colon$Group,
  data=metadata_colon)
